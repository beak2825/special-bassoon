...
var sound = new function() {
    ...
    this.stopSound = function(src) {
        if ((typeof src) == "undefined") {
            console.log("stopSound() error - no sound specified\nProceeding to stop all sound...");
            for (x = 0; x < sounds.length; x++) {
                if (sounds[x].file.paused == false) {
                    sounds[x].file.pause();
                    return console.log("" + sounds[x].name + " is paused/stopped");
                };
            };
            return;
        } else {
            ...
            return console.log("stopSound() error - invalid sound specified.");
        };
    };
