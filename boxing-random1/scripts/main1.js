...
"use strict"; {
    ...
    function b(g, c) {
        ...
        return g.isStringSrc ? new Promise(l => {
    ...
    window.RuntimeInterface = class g {
        constructor(c) {
            ...
            !this._useWorker || "undefined" !== typeof OffscreenCanvas && navigator.userActivation && ("module" !== this._scriptsType || m()) || (this._useWorker = !1);
            f(this._exportType) &&
                this._useWorker && (console.warn("[C3 runtime] Worker mode is enabled and supported, but is disabled in WebViews due to crbug.com/923007. Reverting to DOM mode."), this._useWorker = !1);
            ...
            this.AddRuntimeComponentMessageHandler("runtime",
                "cordova-fetch-local-file", k => this._OnCordovaFetchLocalFile(k));
        ...
        async _Init(c) {
            if ("playable-ad" === this._exportType) {
                ...
                for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
                    ...
                    this._localFileStrings.hasOwnProperty(k) ? c.engineScripts[l] = {
                        isStringSrc: !0,
        ...
        async _InitDOM(c, k) {
            ...
            if ("module" === this._scriptsType) {
                ...
                for (let [t, r] of q)
                    if (r || (r = t), t === l) try {
                        r = await this._MaybeGetCordovaScriptURL(r, this._scriptsType), await b(r, this._scriptsType), "preview" !== this._exportType || n[t] || this._ReportProjectMainScriptError(t, "main script did not run to completion")
                    } catch (y) {
                        this._ReportProjectMainScriptError(t, y)
                    } else if ("scriptsInEvents.js" === t || t.endsWith("/scriptsInEvents.js")) r = await this._MaybeGetCordovaScriptURL(r,
                        this._scriptsType), await b(r, this._scriptsType)
            } else if (c.projectScripts && 0 < c.projectScripts.length) try {
                if (await Promise.all(c.projectScripts.map(q => b(q[1], this._scriptsType))), Object.values(n).some(q => !q)) {
            ...
            "preview" === this._exportType && "object" !== typeof self.C3.ScriptsInEvents ? (this._RemoveLoadingMessage(),
                console.error("[C3 runtime] Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax."), alert("Failed to load JavaScript code used in events. Check all your JavaScript code has valid syntax.")) : (c = Object.assign(this._GetCommonRuntimeOptions(c), {
                isInWorker: !1,
                messagePort: k,
                canvas: this._canvas,
        ...
        _ReportProjectScriptError(c) {
            ...
            c = `Failed to load project script '${Object.entries(c).filter(k=>!k[1]).map(k=>k[0])[0]}'. Check all your JavaScript code has valid syntax.`;
        ...
        _OnEventFromRuntime(c) {
            ...
            if (c = x.get(k))
                if (c = c.get(l)) {
                    ...
                    if (null === q) return t;
                    t && t.then ? t.then(r => this._PostResultToRuntime(q, !0, r)).catch(r => {
        ...
        async _WasmDecodeWebMOpus(c) {
            c = await this.PostToRuntimeComponentAsync("runtime",
                "opus-decode", {
                    arrayBuffer: c
                }, null, [c]);
        ...
        async _MaybeGetCordovaScriptURL(c, k) {
            return "cordova" === this._exportType && "module" === k && (c.startsWith("file:") || "file:" === location.protocol && this.IsRelativeURL(c)) ? (c.startsWith(this._baseUrl) && (c = c.substr(this._baseUrl.length)), c = await this.CordovaFetchLocalFileAsArrayBuffer(c), c = new Blob([c], {
                type: "application/javascript"
        ...
        _HasPermission(c) {
            ...
            return new Promise((l, n) => k.checkPermission(this._MapPermissionID(k, c), q => l(!!q.hasPermission), n))
        }
        _RequestPermission(c) {
            const k = this._GetPermissionAPI();
            return new Promise((l, n) => k.requestPermission(this._MapPermissionID(k, c), q => l(!!q.hasPermission), n))
        }
        ...
        CordovaFetchLocalFile(c) {
            ...
            return new Promise((l, n) => {
                window.resolveLocalFileSystemURL(k, q => {
                    q.file(l, n)
                }, n)
            })
        }
        ...
        async _ConvertDataUriToBlobs(c, k) {
            if ("object" === typeof k) this._localFileBlobs[c] = new Blob([k.str], {
                type: k.type
        ...
        _BinaryStringToBlob(c, k) {
            ...
            let n = l >> 2,
                ...
                r, y;
            for (y = r = 0; r < n; ++r) t[r] =
                c.charCodeAt(y++) | c.charCodeAt(y++) << 8 | c.charCodeAt(y++) << 16 | c.charCodeAt(y++) << 24;
    ...
    d.AddDOMHandlerClass(class extends self.DOMHandler {
        constructor(f) {
            ...
            f.AddRuntimeComponentMessageHandler("runtime",
                "post-to-debugger", k => this._OnPostToDebugger(k));
            ...
            "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", x, {
                passive: !1
            }), c.addEventListener("pointerdown", x)) : c.addEventListener("touchstart", x);
        ...
        Attach() {
            ...
            window.addEventListener("wheel", g => this._OnMouseWheelEvent("wheel",
                g));
            "undefined" !== typeof PointerEvent ? (window.addEventListener("pointerdown", g => {
                ...
                g => this._OnPointerEvent("pointermove", g)), window.addEventListener("pointerup", g => this._OnPointerEvent("pointerup", g)), window.addEventListener("pointercancel", g => this._OnPointerEvent("pointercancel", g))) : (window.addEventListener("mousedown", g => {
                this._HandlePointerDownFocus(g);
                this._OnMouseEventAsPointer("pointerdown", g)
            }), window.addEventListener("mousemove", g => this._OnMouseEventAsPointer("pointermove", g)), window.addEventListener("mouseup", g => this._OnMouseEventAsPointer("pointerup", g)), window.addEventListener("touchstart",
                g => {
                    this._HandlePointerDownFocus(g);
        ...
        _OnWindowResize() {
            ...
            this._iRuntime.IsiOSWebView() && (-1 !== this._simulatedResizeTimerId &&
                clearTimeout(this._simulatedResizeTimerId), this._OnSimulatedResize(f, g, 0))
        }
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen.orientation && screen.orientation.lock) screen.orientation.lock(f).catch(g => console.warn("[Construct 3] Failed to lock orientation: ", g));
                ...
                let g = !1;
                screen.lockOrientation ? g = screen.lockOrientation(f) : screen.webkitLockOrientation ? g = screen.webkitLockOrientation(f) : screen.mozLockOrientation ? g = screen.mozLockOrientation(f) :
        ...
        _OnPointerEvent(f, g) {
            this._pointerRawUpdateRateLimiter && "pointermove" !==
                f && this._pointerRawUpdateRateLimiter.Reset();
            var c = 0;
            ...
            "mouse" === g.pointerType &&
                (c = "mousemove", "pointerdown" === f ? c = "mousedown" : "pointerup" === f && (c = "mouseup"), this._OnMouseEvent(c, g, h), this._mousePointerLastButtons = g.buttons)
        }
        ...
        _HandlePointerDownFocus(f) {
            ...
            this._IsElementCanvasOrDocument(f.target) && document.activeElement && !this._IsElementCanvasOrDocument(document.activeElement) &&
        ...
        async _OnRasterSvgImage(f) {
            ...
            g = g ? await createImageBitmap(f,
                g) : await createImageBitmap(f);
            return {
                imageBitmap: g,
                transferables: [g]
            }
        ...
        _OnDebugHighlight(f) {
            if (f.show) {
                this._debugHighlightElem || (this._debugHighlightElem = document.createElement("div"), this._debugHighlightElem.id =
                    "inspectOutline", document.body.appendChild(this._debugHighlightElem));
        ...
        async _Play(a) {
            ...
            try {
                this._lastAudioInstance =
                    await this._GetAudioInstance(b, e, h, p, m), u ? (this._lastAudioInstance.SetPannerEnabled(!0), this._lastAudioInstance.SetPan(u.x, u.y, u.angle, u.innerAngle, u.outerAngle, u.outerGain), u.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(u.uid)) : this._lastAudioInstance.SetPannerEnabled(!1), this._lastAudioInstance.Play(v, z, A, w)
            } catch (B) {
